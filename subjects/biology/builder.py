...
from subjects.biology.metrics import (
    empty_metrics_biology,
    extract_metrics,
    merge_metrics_biology,
    metrics_to_row,
)
...
_fetch_week_metrics, build_group_all_weeks, _build_report_job = make_builder(
    ...
    merge_metrics_fn=merge_metrics_biology,
    empty_metrics_fn=empty_metrics_biology,
...
async def _process_single_course(course, token, study_month, client, semaphore):
    ...
    try:
        ...
        course_avg = merge_metrics_biology([gr["monthly"] for gr in group_results])
