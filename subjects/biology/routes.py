...
from subjects.biology.metrics import metrics_to_row, compute_avg_row
from subjects.biology.builder import _build_report_job, _build_section_report_job
...
async def fetch_courses_by_type(course_type: str, token: str, stream_month: str = None) -> list:
    ...
    for product in products:
        url = (
            f"{BASE_URL}/v2/headteacher/subjects/{BIOLOGY_SUBJECT_ID}/courses"
...
async def report_result(request: Request):
    ...
    return templates.TemplateResponse("report_biology.html", {
        "request": request,
        "tables": tables,
        "course_name": course_name,
        "study_month": study_month,
...
async def section_report_result(request: Request):
    ...
    REPORT_STORE[report_key] = {
        "tables": [{
            "title": f"Биология {course_type} {study_month}",
            "rows": rows,
            "avg_row": avg_row,
        }],
        "title": f"Биология {course_type} {study_month}",
    ...
    return templates.TemplateResponse("report_biology.html", {
        ...
        "tables": [{
            ...
            "subtitle": f"Биология {course_type} {study_month}",
            "week": "section",
            "rows": rows,
            "avg_row": avg_row,
        }],
        "course_name": f"Биология {course_type}",
        "study_month": study_month,
        "error": None,
...
async def export_csv(request: Request):
    ...
    return Response(
        ...
        headers={
            "Content-Disposition": "attachment; filename=biology_report.csv"
        },
    )
