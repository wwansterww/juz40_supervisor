...
from subjects.chemistry.metrics import metrics_to_row, compute_avg_row
from subjects.chemistry.builder import _build_report_job
...
async def fetch_chemistry_courses_by_type(course_type: str, token: str) -> list:
    products = COURSE_TYPE_TO_PRODUCTS.get(course_type.upper(), [course_type.upper()])

    urls = [
        f"{BASE_URL}/v2/headteacher/subjects/{CHEMISTRY_SUBJECT_ID}/courses"
...
async def chemistry_dashboard(request: Request):
...
async def chemistry_filter_courses(
    request: Request,
    ...
    try:
        all_courses = await fetch_chemistry_courses_by_type(course_type, token)
        ...
        return templates.TemplateResponse("dashboard.html", {
            ...
            "error": "Химия курстарын жүктеу кезінде қате шықты.",
            "subject_name": "Химия",
            "subject_prefix": "/chemistry",
            "active_subject": "chemistry",
            "show_section_report": False,
...
async def chemistry_report_start(
    request: Request,
    course_id: str = Form(...),
    course_name: str = Form(...),
    study_month: str = Form(...),
    ...
    try:
        ...
        return JSONResponse(
            {"error": "Химия топтарын жүктеу кезінде қате шықты."},
            status_code=500,
        )
    ...
    request.session["last_chemistry_job_id"] = job_id
    request.session["last_chemistry_course_name"] = course_name
    request.session["last_chemistry_study_month"] = study_month
...
async def chemistry_report_result(request: Request):
    from main import templates
    ...
    job_id = request.session.get("last_chemistry_job_id")
    ...
    REPORT_STORE[report_key] = {
        ...
        "title": f"Химия · {course_name} {study_month}",
    }

    request.session["last_chemistry_report_key"] = report_key

    return templates.TemplateResponse("chemistry_report.html", {
        "request": request,
        "tables": tables,
        "course_name": f"Химия · {course_name}",
        "study_month": study_month,
        "error": None,
        "group_count": len(group_results),
        "dashboard_url": "/chemistry/dashboard",
        "export_url": "/chemistry/export",
    })


@router.get("/export")
async def chemistry_export_csv(request: Request):
    token = request.session.get("token")
    ...
    if not store:
        return Response(
            content="Экспортқа деректер жоқ. Алдымен химия отчет жасаңыз.",
            status_code=400,
        )
    ...
    return Response(
        ...
        headers={
            "Content-Disposition": "attachment; filename=chemistry_report.csv"
        },
    )


@router.get("/course-months")
async def chemistry_course_months(request: Request, course_id: str):
    token = request.session.get("token")
