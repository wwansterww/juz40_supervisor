...
from subjects.english.metrics import metrics_to_row, compute_avg_row
from subjects.english.builder import _build_report_job, _build_section_report_job
...
async def fetch_courses_by_type(course_type: str, token: str, stream_month: str = None) -> list:
    ...
    for product in products:
        url = (
            f"{BASE_URL}/v2/headteacher/subjects/{ENGLISH_SUBJECT_ID}/courses"
...
async def dashboard(request: Request):
    ...
    return templates.TemplateResponse("dashboard.html", {
        ...
        "subject_name": "Ағылшын тілі",
        "subject_prefix": "/english",
    })
...
async def report_result(request: Request):
    ...
    if not p or p["status"] != "done":
        return RedirectResponse("/english/dashboard", status_code=302)
    ...
    return templates.TemplateResponse("report_english.html", {
        "request": request,
        "tables": tables,
        "course_name": course_name,
        "study_month": study_month,
...
async def export_csv(request: Request):
    ...
    return Response(
        ...
        headers={
            "Content-Disposition": "attachment; filename=english_report.csv"
        },
    )