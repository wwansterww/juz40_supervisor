...
async def fetch_geometry_courses_by_type(course_type: str, token: str) -> list:
    products = COURSE_TYPE_TO_PRODUCTS.get(course_type.upper(), [course_type.upper()])

    urls = [
        f"{BASE_URL}/v2/headteacher/subjects/{GEOMETRY_SUBJECT_ID}/courses"
...
async def geometry_dashboard(request: Request):
...
async def geometry_filter_courses(
    request: Request,
    ...
    try:
        all_courses = await fetch_geometry_courses_by_type(course_type, token)
...
async def math_report_start(
    ...
    request.session["last_geometry_job_id"] = job_id
    request.session["last_geometry_course_name"] = course_name
    request.session["last_geometry_study_month"] = study_month
...
async def math_report_result(request: Request):
    ...
    job_id = request.session.get("last_geometry_job_id")
    ...
    REPORT_STORE[report_key] = {
        ...
        "title": f"Геометрия · {course_name} {study_month}",
    }

    request.session["last_geometry_report_key"] = report_key

    return templates.TemplateResponse("report.html", {
        "request": request,
        "tables": tables,
        "course_name": f"Геометрия · {course_name}",
        "study_month": study_month,
        "error": None,
        "group_count": len(group_results),

        "dashboard_url": "/geometry/dashboard",
        "export_url": "/geometry/export",
    })
...
async def math_export_csv(request: Request):
    ...
    return Response(
        ...
        headers={
            "Content-Disposition": "attachment; filename=geometry_report.csv"
        },
    )
