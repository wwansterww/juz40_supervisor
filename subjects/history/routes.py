...
async def report_result(request: Request):
    ...
    if not p or p["status"] != "done":
        return RedirectResponse("/history/dashboard", status_code=302)
...
async def section_report_result(request: Request):
    ...
    report_key = job_id
    REPORT_STORE[report_key] = {
        "tables": [{"title": f"Тарих {course_type} {study_month}", "rows": rows, "avg_row": avg_row}],
        "title": f"Тарих {course_type} {study_month}",
    }
    request.session["last_report_key"] = report_key
    ...
    return templates.TemplateResponse("report_history.html", {
        ...
        "tables": [{
            ...
            "subtitle": f"Тарих {course_type} {study_month}",
            "week": "section",
            "rows": rows,
            "avg_row": avg_row,
        }],
        "course_name": f"Тарих {course_type}",
        "study_month": study_month,
        "error": None,
