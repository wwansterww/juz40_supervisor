...
from subjects.kazakh_language.metrics import metrics_to_row, compute_avg_row
from subjects.kazakh_language.builder import _build_report_job, _build_section_report_job
...
async def fetch_courses_by_type(course_type: str, token: str, stream_month: str = None) -> list:
    ...
    for product in products:
        url = (
            f"{BASE_URL}/v2/headteacher/subjects/{KAZAKH_LANGUAGE_SUBJECT_ID}/courses"
    ...
    async with httpx.AsyncClient() as client:
        results = await asyncio.gather(
            *[
                fetch_all_course_pages(url, token, client)
...
async def dashboard(request: Request):
    ...
    return templates.TemplateResponse("dashboard.html", {
        ...
        "subject_prefix": "/kazakh-language",
    })
...
async def report_result(request: Request):
    ...
    if not p or p["status"] != "done":
        return RedirectResponse("/kazakh-language/dashboard", status_code=302)
    ...
    return templates.TemplateResponse("report_kazakh_language.html", {
        "request": request,
        "tables": tables,
        "course_name": course_name,
        "study_month": study_month,
...
async def section_report_start(
    ...
    try:
        ...
        courses = [
            c for c in courses
            if "(КОПИЯ" not in c["name"].upper()
        ]
    except Exception:
        return JSONResponse(
            {"error": "Курстарды жүктеу кезінде қате."},
            status_code=500,
        )
...
async def section_report_result(request: Request):
    ...
    if not p or p["status"] != "done":
        return RedirectResponse("/biology/dashboard", status_code=302)
    ...
    REPORT_STORE[report_key] = {
        "tables": [{
            "title": f"Қазақ тілі {course_type} {study_month}",
            "rows": rows,
            "avg_row": avg_row,
        }],
        "title": f"Қазақ тілі {course_type} {study_month}",
    ...
    return templates.TemplateResponse("report_kazakh_language.html", {
        ...
        "tables": [{
            ...
            "subtitle": f"Қазақ тілі {course_type} {study_month}",
            "week": "section",
            "rows": rows,
            "avg_row": avg_row,
        }],
        "course_name": f"Қазақ тілі {course_type}",
        "study_month": study_month,
        "error": None,
...
async def export_csv(request: Request):
    ...
    return Response(
        ...
        headers={
            "Content-Disposition": "attachment; filename=kazakh_language_report.csv"
        },
    )
