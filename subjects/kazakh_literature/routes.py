...
from subjects.kazakh_literature.metrics import metrics_to_row, compute_avg_row
from subjects.kazakh_literature.builder import _build_report_job, _build_section_report_job
...
async def fetch_courses_by_type(course_type: str, token: str, stream_month: str = None) -> list:
    ...
    for product in products:
        url = (
            f"{BASE_URL}/v2/headteacher/subjects/{KAZAKH_LITERATURE_SUBJECT_ID}/courses"
...
async def dashboard(request: Request):
    ...
    return templates.TemplateResponse("dashboard.html", {
        ...
        "subject_prefix": "/kazakh-literature",
    })
...
async def report_result(request: Request):
    ...
    if not p or p["status"] != "done":
        return RedirectResponse("/kazakh-literature/dashboard", status_code=302)
    ...
    return templates.TemplateResponse("report_kazakh_literature.html", {
        "request": request,
        "tables": tables,
        "course_name": course_name,
        "study_month": study_month,
...
async def section_report_result(request: Request):
    ...
    REPORT_STORE[report_key] = {
        "tables": [{
            "title": f"Қазақ әдебиеті {course_type} {study_month}",
            "rows": rows,
            "avg_row": avg_row,
        }],
        "title": f"Қазақ әдебиеті {course_type} {study_month}",
    ...
    return templates.TemplateResponse("report_kazakh_literature.html", {
        ...
        "tables": [{
            ...
            "subtitle": f"Қазақ әдебиеті {course_type} {study_month}",
            "week": "section",
            "rows": rows,
            "avg_row": avg_row,
        }],
        "course_name": f"Қазақ әдебиеті {course_type}",
        "study_month": study_month,
        "error": None,
...
async def export_csv(request: Request):
    ...
    return Response(
        ...
        headers={
            "Content-Disposition": "attachment; filename=kazakh_literature_report.csv"
        },
    )
