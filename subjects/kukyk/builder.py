...
from subjects.kukyk.metrics import empty_metrics_kukyk, extract_metrics, merge_metrics_kukyk, metrics_to_row
import asyncio
import httpx
...
_fetch_week_metrics, build_group_all_weeks, _build_report_job = make_builder(
    ...
    merge_metrics_fn=merge_metrics_kukyk,
    empty_metrics_fn=empty_metrics_kukyk,
)
...
async def _process_single_course(course, token, study_month, client):
    ...
    try:
        ...
        if not group_results:
            return None
        course_avg = merge_metrics_kukyk([gr["monthly"] for gr in group_results])
