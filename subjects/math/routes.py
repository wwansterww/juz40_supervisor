...
from subjects.math.metrics import metrics_to_row, compute_avg_row
...
async def fetch_math_courses_by_type(course_type: str, token: str) -> list:
    products = COURSE_TYPE_TO_PRODUCTS.get(course_type.upper(), [course_type.upper()])

    urls = [
        f"{BASE_URL}/v2/headteacher/subjects/{MATH_SUBJECT_ID}/courses"
...
async def math_dashboard(request: Request):
...
async def math_filter_courses(
    ...
    try:
        all_courses = await fetch_math_courses_by_type(course_type, token)
        ...
        return templates.TemplateResponse("dashboard.html", {
            ...
            "error": "Математика курстарын жүктеу кезінде қате шықты.",
...
async def math_report_start(
    ...
    try:
        ...
        return JSONResponse(
            {"error": "Математика топтарын жүктеу кезінде қате шықты."},
            status_code=500,
        )
    ...
    request.session["last_math_job_id"] = job_id
    request.session["last_math_course_name"] = course_name
    request.session["last_math_study_month"] = study_month
...
async def math_report_result(request: Request):
    from main import templates
    ...
    job_id = request.session.get("last_math_job_id")
    course_name = request.session.get("last_math_course_name", "")
    study_month = request.session.get("last_math_study_month", "")

    p = PROGRESS.get(job_id) if job_id else None

    if not p or p["status"] != "done":
        return RedirectResponse("/math/dashboard", status_code=302)
    ...
    REPORT_STORE[report_key] = {
        ...
        "title": f"Математика · {course_name} {study_month}",
    }

    request.session["last_math_report_key"] = report_key

    return templates.TemplateResponse("report.html", {
        "request": request,
        "tables": tables,
        "course_name": f"Математика · {course_name}",
        "study_month": study_month,
        "error": None,
        "group_count": len(group_results),

        "dashboard_url": "/math/dashboard",
        "export_url": "/math/export",
    })


@router.get("/export")
async def math_export_csv(request: Request):
    token = request.session.get("token")

    if not token:
        return RedirectResponse("/", status_code=302)

    report_key = request.session.get("last_math_report_key")
    store = REPORT_STORE.get(report_key) if report_key else None

    if not store:
        return Response(
            content="Экспортқа деректер жоқ. Алдымен математика отчет жасаңыз.",
            status_code=400,
        )
    ...
    return Response(
        ...
        headers={
            "Content-Disposition": "attachment; filename=math_report.csv"
        },
    )


@router.get("/course-months")
async def math_course_months(request: Request, course_id: str):
    token = request.session.get("token")
