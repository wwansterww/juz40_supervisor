...
from subjects.ms.metrics import metrics_to_row, compute_avg_row
...
async def fetch_ms_courses_by_type(course_type: str, token: str) -> list:
    products = COURSE_TYPE_TO_PRODUCTS.get(course_type.upper(), [course_type.upper()])

    urls = [
        f"{BASE_URL}/v2/headteacher/subjects/{MS_SUBJECT_ID}/courses"
...
async def ms_dashboard(request: Request):
    ...
    return templates.TemplateResponse("dashboard.html", {
        ...
        "subject_name": "Математикалық сауаттылық",
        "subject_prefix": "/ms",
        "active_subject": "ms",
...
async def ms_filter_courses(
    ...
    try:
        all_courses = await fetch_ms_courses_by_type(course_type, token)
        ...
        return templates.TemplateResponse("dashboard.html", {
            ...
            "error": "Математикалық сауаттылық курстарын жүктеу кезінде қате шықты.",
...
async def ms_report_start(
    ...
    try:
        ...
        return JSONResponse(
            {"error": "Математикалық сауаттылық топтарын жүктеу кезінде қате шықты."},
            status_code=500,
        )
    ...
    request.session["last_ms_job_id"] = job_id
    request.session["last_ms_course_name"] = course_name
    request.session["last_ms_study_month"] = study_month

    asyncio.create_task(
        _build_report_job(job_id, groups, token, month_num)
    )
...
async def ms_report_progress(job_id: str):
...
async def ms_report_result(request: Request):
    from main import templates
    ...
    job_id = request.session.get("last_ms_job_id")
    course_name = request.session.get("last_ms_course_name", "")
    study_month = request.session.get("last_ms_study_month", "")

    p = PROGRESS.get(job_id) if job_id else None

    if not p or p["status"] != "done":
        return RedirectResponse("/ms/dashboard", status_code=302)
    ...
    REPORT_STORE[report_key] = {
        ...
        "title": f"Математикалық сауаттылық · {course_name} {study_month}",
    }

    request.session["last_ms_report_key"] = report_key

    return templates.TemplateResponse("report.html", {
        "request": request,
        "tables": tables,
        "course_name": f"Математикалық сауаттылық · {course_name}",
        "study_month": study_month,
        "error": None,
        "group_count": len(group_results),

        "dashboard_url": "/ms/dashboard",
        "export_url": "/ms/export",
    })


@router.get("/export")
async def ms_export_csv(request: Request):
    token = request.session.get("token")

    if not token:
        return RedirectResponse("/", status_code=302)

    report_key = request.session.get("last_ms_report_key")
    store = REPORT_STORE.get(report_key) if report_key else None

    if not store:
        return Response(
            content="Экспортқа деректер жоқ. Алдымен Математикалық сауаттылық отчет жасаңыз.",
            status_code=400,
        )
    ...
    if not tables:
        return Response(
            content="Экспортқа деректер жоқ",
            status_code=400,
        )
    ...
    return Response(
        ...
        headers={
            "Content-Disposition": "attachment; filename=ms_report.csv"
        },
    )


@router.get("/course-months")
async def ms_course_months(request: Request, course_id: str):
    token = request.session.get("token")
