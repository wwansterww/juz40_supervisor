...
from subjects.physics.metrics import (
    empty_metrics, extract_metrics, merge_metrics,
    metrics_to_row,
)
...
async def _fetch_week_metrics(
    ...
    """Бір апта үшін барлық тақырып метрикаларын қайтарады."""
    try:
...
async def build_group_all_weeks(
    ...
    # Студент саны + 4 апта — бәрін бірден параллель жіберіп аламыз
...
async def _process_single_course(
    ...
    try:
        ...
        monthly_metrics = [gr["monthly"] for gr in group_results]
        course_avg = merge_metrics(monthly_metrics)
        total_students = sum(
...
async def _build_section_report_job(
    job_id: str,
    courses: list,
    token: str,
    study_month: int,
    ...
    async with httpx.AsyncClient(limits=CLIENT_LIMITS) as client:
        results = []
        for i in range(0, total, course_batch_size):
            batch = courses[i: i + course_batch_size]
            batch_results = await asyncio.gather(
            ...
            for r in batch_results:
                if not isinstance(r, Exception) and r is not None:
                    results.append(r)
            PROGRESS[job_id]["done"] = min(i + course_batch_size, total)
