...
from subjects.russian_language.metrics import metrics_to_row, compute_avg_row
from subjects.russian_language.builder import _build_report_job, _build_section_report_job
...
async def fetch_courses_by_type(course_type: str, token: str, stream_month: str = None) -> list:
    ...
    for product in products:
        url = (
            f"{BASE_URL}/v2/headteacher/subjects/{RUSSIAN_LANGUAGE_SUBJECT_ID}/courses"
...
async def dashboard(request: Request):
    ...
    return templates.TemplateResponse("dashboard.html", {
        ...
        "subject_name": "Орыс тілі",
        "subject_prefix": "/russian-language",
    })
...
async def report_result(request: Request):
    ...
    if not p or p["status"] != "done":
        return RedirectResponse("/russian-language/dashboard", status_code=302)
    ...
    return templates.TemplateResponse("report_russian_language.html", {
        "request": request,
        "tables": tables,
        "course_name": course_name,
        "study_month": study_month,
...
async def section_report_result(request: Request):
    ...
    REPORT_STORE[report_key] = {
        "tables": [{
            "title": f"Орыс тілі {course_type} {study_month}",
            "rows": rows,
            "avg_row": avg_row,
        }],
        "title": f"Орыс тілі {course_type} {study_month}",
    ...
    return templates.TemplateResponse("report_russian_language.html", {
        ...
        "tables": [{
            ...
            "subtitle": f"Орыс тілі {course_type} {study_month}",
            "week": "section",
            "rows": rows,
            "avg_row": avg_row,
        }],
        "course_name": f"Орыс тілі {course_type}",
        "study_month": study_month,
        "error": None,
...
async def export_csv(request: Request):
    ...
    return Response(
        ...
        headers={
            "Content-Disposition": "attachment; filename=russian_language_report.csv"
        },
    )
