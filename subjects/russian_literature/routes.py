...
from subjects.russian_literature.builder import (
    _build_report_job,
...
def matches_type(name: str, course_type: str) -> bool:
    ...
    keywords = TYPE_NAME_KEYWORDS.get(
        course_type.upper(),
        [course_type.upper()],
    )
...
async def fetch_courses_by_type(
    course_type: str,
    token: str,
    stream_month: str = None,
) -> list:

    products = COURSE_TYPE_TO_PRODUCTS.get(
        course_type.upper(),
        [course_type.upper()],
    )

    month_num = MONTH_NAME_TO_NUM.get(
        (stream_month or "").upper()
    )
    ...
    for product in products:
        url = (
            f"{BASE_URL}/v2/headteacher/subjects/"
            f"{RUSSIAN_LITERATURE_SUBJECT_ID}/courses"
            f"?size=50&page=0"
            f"&searchWord="
            f"&sort=year,DESC"
            f"&sort=month,DESC"
...
async def dashboard(request: Request):
    ...
    return templates.TemplateResponse(
        "dashboard.html",
        {
            ...
            "subject_name": "Орыс әдебиеті",
            "subject_prefix": "/russian-literature",
        },
    )
...
async def filter_courses(
    ...
    try:
        all_courses = await fetch_courses_by_type(
            course_type,
            token,
            stream_month,
        )
        ...
        filtered = [
            ...
            if (
                ...
                or (
                    month_num is not None
                    and c.get("month") == month_num
                )
            )
...
async def report_start(
    ...
    try:
        ...
        return JSONResponse(
            {"error": "Топтарды жүктеу кезінде қате."},
            status_code=500,
        )

    if not groups:
        return JSONResponse(
            {"error": "Топтар табылмады."},
            status_code=404,
        )
...
async def report_result(request: Request):
    ...
    for week in range(1, 5):
        ...
        tables.append(
            {
                "title": f"{week}-апта",
                "subtitle": f"{study_month} {week}-апта",
                "week": week,
                "rows": rows,
                "avg_row": avg_row,
            }
    ...
    tables.append(
        {
            ...
            "subtitle": f"{study_month} қорытындысы",
            "week": "monthly",
            "rows": monthly_rows,
            "avg_row": monthly_avg,
        }
    ...
    return templates.TemplateResponse(
        "report_russian_literature.html",
        {
            "request": request,
            "tables": tables,
            "course_name": course_name,
...
async def export_csv(request: Request):
    ...
    return Response(
        ...
        headers={
            "Content-Disposition":
            "attachment; filename=russian_literature_report.csv"
        },
    )