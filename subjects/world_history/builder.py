...
from subjects.world_history.metrics import (
    empty_metrics_world_history,
    extract_metrics,
    merge_metrics_world_history,
    metrics_to_row,
)
...
_fetch_week_metrics, build_group_all_weeks, _build_report_job = make_builder(
    ...
    merge_metrics_fn=merge_metrics_world_history,
    empty_metrics_fn=empty_metrics_world_history,
)
    ...
    try:
        ...
        for j in range(0, len(groups), 10):
            ...
            batch_results = await asyncio.gather(
                *[
                    build_group_all_weeks(g, token, study_month, client)
                    for g in batch
                ],
        ...
        course_avg = merge_metrics_world_history([
...
async def _build_section_report_job(job_id, courses, token, study_month):
    ...
    async with httpx.AsyncClient(limits=CLIENT_LIMITS) as client:
        ...
        for i in range(0, total, 5):
            ...
            batch_results = await asyncio.gather(
                *[
                    _process_single_course(c, token, study_month, client)
                    for c in batch
                ],
                return_exceptions=True,
            )
