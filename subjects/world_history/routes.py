...
from subjects.world_history.metrics import metrics_to_row, compute_avg_row
from subjects.world_history.builder import _build_report_job, _build_section_report_job
...
async def fetch_courses_by_type(course_type: str, token: str, stream_month: str = None) -> list:
    ...
    for product in products:
        url = (
            f"{BASE_URL}/v2/headteacher/subjects/{WORLD_HISTORY_SUBJECT_ID}/courses"
...
async def dashboard(request: Request):
    ...
    return templates.TemplateResponse("dashboard.html", {
        ...
        "subject_name": "Дүниежүзі тарихы",
        "subject_prefix": "/world-history",
    })
...
async def report_result(request: Request):
    ...
    if not p or p["status"] != "done":
        return RedirectResponse("/world-history/dashboard", status_code=302)
...
async def section_report_result(request: Request):
    ...
    REPORT_STORE[report_key] = {
        "tables": [{
            "title": f"Дүниежүзі тарихы {course_type} {study_month}",
            "rows": rows,
            "avg_row": avg_row,
        }],
        "title": f"Дүниежүзі тарихы {course_type} {study_month}",
    ...
    return templates.TemplateResponse("report_geography.html", {
        ...
        "tables": [{
            ...
            "subtitle": f"Дүниежүзі тарихы {course_type} {study_month}",
            "week": "section",
            "rows": rows,
            "avg_row": avg_row,
        }],
        "course_name": f"Дүниежүзі тарихы {course_type}",
        "study_month": study_month,
        "error": None,
...
async def export_csv(request: Request):
    ...
    return Response(
        ...
        headers={
            "Content-Disposition": "attachment; filename=world_history_report.csv"
        },
    )
